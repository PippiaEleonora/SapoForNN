...
if dataset=='acasxu':
    ...
    for i, test in enumerate(tests):
        ...
        #Note: the 3' argument -> init_domain(domain) is changing 'refinepoly' into 'deeppoly'
        #We are just extrapolate the label of the current test
        ...
        if(label == int(test[0])):
            ...
            if config.normalized_region==True:
                #here we create an interval input
            ...
            if config.target == None:
                ...
                prop = int(target[i])
            #We are analyzing again the NN having a non-trivial input domain
            #Now domain='refinepoly'
